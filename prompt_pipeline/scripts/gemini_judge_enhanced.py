#!/usr/bin/env python3
"""
Judge enhanced prompts via Gemini API: score all dimension groups (A–D) including aesthetics.
Input: JSONL with "prompt_id" and "enhanced_prompt" (e.g. enhanced_final.jsonl). Output: same + dimension scores.
No flag-for-review logic: enhanced prompts are assumed already vetted.
"""
...
JUDGE_SYSTEM = """You are a video generation benchmarking expert. Analyze the enhanced prompts and provide scores on multiple dimensions.
...
# Output key order: ids/enhanced_prompt first, then dimension groups, then categories/confidence
OUTPUT_KEY_ORDER = [
    "prompt_id", "enhanced_prompt",
    "applicable_groups",
...
def main():
    ...
    ap.add_argument("--input", required=True, help="Input JSONL path (prompt_id + enhanced_prompt, e.g. enhanced_final.jsonl)")
    ap.add_argument("--output", required=True, help="Output JSONL path")
    ap.add_argument("--prompt-field", default="enhanced_prompt", help="Field name for prompt text (default: enhanced_prompt for enhanced_final.jsonl)")
