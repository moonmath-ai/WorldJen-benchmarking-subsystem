"""
generate_dragon_appendix.py

Produces two appendix figures for prompt_1732 (the "clean" case study):

  dragon_frame_grid.pdf   — 6×6 grid of one representative frame per model
  dragon_vlm_scores.pdf   — heatmap of per-question Physical Mechanics
                            Likert scores across all six models

Run:  python generate_dragon_appendix.py
"""
...
PROMPT_ID = "prompt_1732"

# ── Model registry ────────────────────────────────────────────────────────────
MODEL_ORDER = [
    ("fal-ai_veo3.1_fast",
     "Veo 3.1",
     "fal-ai_veo3.1_fast_prompt_1732"),
    ("fal-ai_kling-video_v2.6_pro_text-to-video",
     "Kling v2.6",
     "fal-ai_kling-video_v2.6_pro_text-to-video_prompt_1732"),
    ("fal-ai_wan_v2.2-a14b_text-to-video",
     "Wan A14B",
     "fal-ai_wan_v2.2-a14b_text-to-video_prompt_1732"),
    ("fal-ai_ltx-2_text-to-video",
     "LTX-2",
     "fal-ai_ltx-2_text-to-video_prompt_1732"),
    ("fal-ai_hunyuan-video-v1.5_text-to-video",
     "Hunyuan",
     "fal-ai_hunyuan-video-v1.5_text-to-video_prompt_1732"),
    ("wan2.1-1.3b",
     "Wan 1.3B",
     "wan2.1-1.3b_prompt_1732"),
]

DIM_KEY = "physical_mechanics"

# Short question labels matching the VQA set for prompt_1732 (Physical Mechanics)
Q_LABELS = [
    "Q1:  Dragon deceleration / weight inertia",
    "Q2:  Sparks originate from sword contact point",
    "Q3:  Floor deformation from dragon's mass",
    "Q4:  Foot friction / grip on stone floor",
    "Q5:  Flames displaced by body movement",
    "Q6:  Sword slash arc — arm range & momentum",
    "Q7:  Spark dissipation / ballistic fall",
    "Q8:  Armor mass — micro-shifts on sudden stop",
    "Q9:  Blade contacts floor (no clipping)",
    "Q10: Shadow movement matches flames & pillars",
]
...
# Timestamps chosen to capture the lunge arc for physical mechanics:
#   t=10% (approach) → 25% → 40% (lunge) → 55% (impact) → 70% → 85% (post-halt)
SAMPLE_POINTS = [0.10, 0.25, 0.40, 0.55, 0.70, 0.85]


def get_frames(vid_path: pathlib.Path, points: list = SAMPLE_POINTS) -> list:
...
def load_pm_scores(json_stem: str) -> list | None:
    ...
    return [e["score"] for e in entries if e and e.get("score") is not None]
...
def make_frame_grid():
    ...
    for mk, short, jstem in MODEL_ORDER:
        ...
        sc = load_pm_scores(jstem)
        pm_avgs.append(np.mean(sc) if sc else 0.0)
    ...
    for mi, (short, pm_avg) in enumerate(zip(model_labels, pm_avgs)):
        ax_hdr = fig.add_subplot(gs[0, mi])
        ax_hdr.axis("off")
        badge_col = CMAP((pm_avg - 1) / 4)
        ...
        ax_hdr.text(0.5, 0.22, f"PM = {pm_avg:.1f} / 5",
    ...
    for fi in range(N_FRAMES):
        for mi in range(N_MODELS):
            ...
            ax.axis("off")
            ax.imshow(frames_by_model[mi][fi], aspect="auto")
            if mi == 0:
    ...
    fig.suptitle(
        "Prompt 1732 — Frames sampled across full video duration per model  "
        "(rows = timestamps;  header colour = Physical Mechanics score)",
        fontsize=10, y=0.975)

    fig.savefig(OUT / "dragon_frame_grid.pdf", bbox_inches="tight", dpi=150)
    plt.close(fig)
    print("Saved → dragon_frame_grid.pdf")


# ══════════════════════════════════════════════════════════════════════════════
# Figure B — per-question Physical Mechanics score heatmap
# ══════════════════════════════════════════════════════════════════════════════
def make_vlm_score_table():
    ...
    for _, _, jstem in MODEL_ORDER:
        sc = load_pm_scores(jstem)
        score_matrix.append(sc if sc and len(sc) == 10 else [np.nan] * 10)
    score_matrix = np.array(score_matrix, dtype=float).T   # (10, 6)

    avg_row = np.nanmean(score_matrix, axis=0)   # (6,)

    # ── Layout axes ───────────────────────────────────────────────────────────
    LS, LB_s, LW_s, LH_s = 0.01, 0.14, 0.022, 0.78
    LL, LB,   LW,   LH   = 0.034, 0.14, 0.24,  0.78
    HL, HB,   HW,   HH   = 0.278, 0.14, 0.68,  0.78
    AL, AB,   AW,   AH   = 0.278, 0.07, 0.68,  0.055
    CL, CB,   CW,   CH   = 0.278, 0.01, 0.68,  0.025
    ...
    ax_stripe.add_patch(plt.Rectangle((0, -0.5), 1, N_Q,
                                      facecolor="#4E5FA8", edgecolor="none"))
    ax_stripe.text(0.5, N_Q / 2 - 0.5, "Physical\nMechanics",
    ...
    ax_avg.imshow(avg_row.reshape(1, -1), aspect="auto",
                  cmap=CMAP, vmin=1, vmax=5, origin="upper")
    ax_avg.set_xticks([])
    ax_avg.set_yticks([0])
    ...
    fig.suptitle(
        "Physical Mechanics — Per-Question VLM Likert Scores  "
        f"(Prompt 1732 · Clean Example · 10 questions × 6 models)",
        fontsize=11, y=0.995)

    out_path = OUT / "dragon_vlm_scores.pdf"
